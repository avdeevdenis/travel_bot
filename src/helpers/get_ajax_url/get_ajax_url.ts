...
const DEFAULT_CITIES = process.env.DEFAULT_CITIES;
const DEFAULT_COUNTRIES = process.env.DEFAULT_COUNTRIES as unknown as AjaxOptions['countries'];
const DEFAULT_KHOT = 30;
...
export const getAjaxUrl = (options?: AjaxOptions) => {
    ...
    const countriesString = countries.split(',').map(country => `&countries[]=${country.trim()}`).join('');
